			  ...
			  <tbody>
			  	...
			  	<tr>
			  		...
			  		<td rowspan="{{count($group->products)}}">
			  			<input id="group_url_{{$i}}" style="" type="text" value="{{env('APP_URL').'/dealer/'.$group->group_code}}">
			  			<button class="btn btn-sm btn-primary" onclick="copy('group_url_{{$i}}')">複製</button>
			  			<a class="btn btn-sm btn-success" href="{{env('APP_URL').'/dealer/'.$group->group_code}}" target="_blank">瀏覽</a>
			  		</td>
